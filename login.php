...
<html lang="fr">
  ...
  <body>
    ...
    <div class="auth-container reveal">
      ...
      <form method="post" action="<?= h(url('login.php')) ?>" novalidate>
        <input type="hidden" name="csrf_token" value="<?= h(generate_csrf_token()) ?>">
        <div class="field-group">
          <label for="email">Email</label>
          <input type="email" id="email" name="email" value="<?= htmlspecialchars($oldEmail, ENT_QUOTES, 'UTF-8') ?>" required placeholder="votre@email.com" />
        </div>
      ...
      <div class="auth-footer">
        Pas encore membre ? <a href="<?= url('register.php') ?>">Créer un compte</a>
      </div>
    </div>

    <script>
      /**
       * UNDR CLUB - Login UI Logic
